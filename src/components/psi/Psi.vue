<template>
  <svg @pointermove.self="onPointer" width="100%" version="1.1" :viewBox="svgToDisplay.viewBox"
    xmlns="http://www.w3.org/2000/svg">
    <g ref="psi" fill="#333333">
      <circle v-for="({ circle, animation }, index) in circlesListAnimated" :cx="circle.cx" :cy="circle.cy"
        :r="circle.r">
        <animate :attributeName="animation.attributeName" :values="animation.values" :dur="animation.duration"
...
export default defineComponent({
  props: {
    ...
    viewBox: {
      type: String,
      default: '0 0 119.06 119.06',
    },
    circlesList: {
      type: <{ cx: String, cy: String, r: String }>[],
      default: []
    },
    preset: {
      type: String,
  ...
  setup(props, ctx) {
    ...
    const svgToDisplay = computed(() => {
      ...
      if (props.circlesList.length > 0) {
        return {
          width: props.width,
          height: props.height,
          viewBox: props.viewBox,
          circlesList: props.circlesList,
        }
      }
